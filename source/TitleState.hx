...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// music.loadStream(Paths.music('itsRainingErrors'));
			// FlxG.sound.list.add(music);
			// music.play();

			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('itsRainingErrors'), 0);
		...
		logoSpr.setGraphicSize(Std.int(logoSpr.width * 0.60));
		...
		if(!closedState) {
			switch (curBeat)
			{
				...
				case 8:
				    addMoreText('Tacos');
				case 12:
				    addMoreText('This is so nostalgic');
                case 14:
				    deleteCoolText();
                case 16:
				    createCoolText(['A mod about']);
				case 20:
				    addMoreText('The shitty error at');
