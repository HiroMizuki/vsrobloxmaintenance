...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'studios':
				var bg:BGSprite = new BGSprite('studios/BG', 0, 100, 0.97, 0.97);
				bg.setGraphicSize(Std.int(bg.width * 2.00));
				add(bg);

				var floor:BGSprite = new BGSprite('studios/Floor', 0, 800, 0.9, 0.9);
				floor.setGraphicSize(Std.int(floor.width * 1.85));
				add(floor);

				var monitor:BGSprite = new BGSprite('studios/Monitor_Roblox_Cancelled_by_Arch', 550, 100, 0.97, 0.97);
				monitor.setGraphicSize(Std.int(monitor.width * 1.35));
				add(monitor);
				
				var monitorFloor1:BGSprite = new BGSprite('studios/1', 300, 380, 0.95, 0.95);
				monitorFloor1.setGraphicSize(Std.int(monitorFloor1.width * 1.30));
				add(monitorFloor1);
				
				var monitorFloor2:BGSprite = new BGSprite('studios/2', 202, 400, 0.95, 0.95);
				monitorFloor2.setGraphicSize(Std.int(monitorFloor2.width * 1.30));
				add(monitorFloor2);

				monitorFloor3 = new BGSprite('studios/3', -250, 700, 0.9, 0.9);
				monitorFloor3.setGraphicSize(Std.int(monitorFloor3.width * 1.40));
		}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
					FlxG.sound.playMusic(Paths.music('itsRainingErrors'));
